...
As Figuras \ref{figura:resultado-temas} e \ref{figura:resultado-sons} apresentam as interfaces dedicadas à customização da experiência do jogador. 

Diferente do rascunho inicial, a implementação final utiliza o \textit{script} \texttt{tela\_temas.gd} (Figura \ref{figura:resultado-temas}) para alternar instantaneamente entre assets (como o tema "Clássico" e "Moderno"). A modularização dessa tela garante que a escolha seja salva globalmente e aplicada ao tabuleiro de forma dinâmica.

\figura{./imagens/menu_temas.png}{12cm}{Interface final de seleção de temas visuais.}{figura:resultado-temas}
...
\figura{./imagens/menu_sons.png}{12cm}{Interface final de controle de canais de áudio.}{figura:resultado-sons}

\subsection{Tabuleiro e Gameplay}

A Figura \ref{figura:resultado-jogo} exibe a tela de jogo principal. O refinamento visual incluiu a renderização de peças personalizadas e a implementação de sinais visuais de seleção. O uso da classe \texttt{base\_xadrez.gd} permitiu que a lógica de movimentação respondesse de forma fluida, com o realce das casas de origem e destino, facilitando a compreensão do fluxo da partida pelo usuário.

\figura{./imagens/tabuleiro_arabe.png}{12cm}{Interface de gameplay com aplicação de temas e realce de movimentos.}{figura:resultado-jogo}
...
Os resultados, consolidados na Tabela \ref{tabela:resultados-desempenho}, demonstram o tempo médio entre o envio da string FEN e o recebimento da jogada UCI, sob uma taxa de atualização constante de 60 FPS.
